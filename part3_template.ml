...
let rec exec (t : term) : string = 
  match t with
  | Int s -> " Push " ^ string_of_int s
  | Unit -> " Push () "
  | Name s -> " Push " ^ s ^ " Lookup "
  | Add (c1, c2) -> (exec c1) ^ (exec c2) ^ " Add "
  | Sub (c1, c2) -> (exec c1) ^ (exec c2) ^ " Sub "
  | Mul (c1, c2) -> (exec c1) ^ (exec c2) ^ " Mul "
  | Div (c1, c2) -> (exec c1) ^ (exec c2) ^ " Div "
  | Ifgz (a, b, c) -> (exec a) ^ " If " ^ " Begin " ^ (exec b) ^ " End " ^
  " Else " ^ " Begin " ^ (exec c) ^ " End " ^ " End "
  | LetIn (n, a, b) -> " Begin " ^ " Push " ^ n ^ (exec a) ^ " Let " ^ (exec b) ^ " End "
  | Trace t -> (exec t) ^ " Trace "
  | Fun (a, b, c) -> " Fun " ^ a ^ " " ^ b ^ (exec c) ^ " End " ^ " Push " ^ a ^ " Lookup "
  | App (a, b) -> (exec a) ^ (exec b) ^ " Call "
